...
class OthelloGUI:
    ...
    def setup_ui(self):
        ...
        # --- ★ハンデ選択UIの追加 ---
        handicap_frame = tk.Frame(control_frame)
        handicap_frame.pack(pady=2)
        tk.Label(handicap_frame, text="ハンデ (最初から角を取る枚数):", font=("Arial", 10)).pack(side=tk.LEFT)
        
        self.handicap_var = tk.IntVar(value=0)
        handicap_menu = tk.OptionMenu(handicap_frame, self.handicap_var, 0, 1, 2, 3, 4)
        handicap_menu.config(font=("Arial", 10))
        handicap_menu.pack(side=tk.LEFT, padx=5)
        # -----------------------------
    ...
    def start_game(self, human_color):
        ...
        # 盤面リセット（この直後は必ず「黒番」の視点になる）
        self.env.reset()

        # --- ★ハンデ（角石の配置）の適用 ---
        h_count = self.handicap_var.get()
        if h_count > 0:
            # 配置する角の順番: 左上(A1=0), 右下(H8=63), 左下(A8=56), 右上(H1=7)
            corners = [0, 63, 56, 7]
            for i in range(h_count):
                coord = np.int8(corners[i])
                # リセット直後は「黒番」なので、人間が黒ならPlayerに、白ならOpponentに石を配置する
                if self.human_color == pyrev.BLACK:
                    self.env.position.put_player_disc_at(coord)
                else:
                    self.env.position.put_opponent_disc_at(coord)
        # -----------------------------------

        self.game_started = True
        self.is_ai_thinking = False
        
        # ゲーム中はボタンとハンデメニューを無効化
    ...
    def update_eval_bar(self):
        ...
        self.eval_canvas.coords(self.eval_rect, 0, 0, int(self.bar_width * win_rate), 22)
