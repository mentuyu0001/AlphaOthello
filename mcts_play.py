...
class MCTS_Play:
    ...
    def search(self, env, time_limit_sec=3.0, is_self_play=False):
        """
        time_limit_sec (float): この関数が処理を終えるまでの目標時間（秒）
        """
        start_time = time.time()
        # 安全マージン（バッファ）として0.05秒を設定
        buffer_sec = 0.05
        end_time = start_time + time_limit_sec - buffer_sec
        ...
        # ★ ここが最大の変更点：回数ではなく「現在時刻が終了予定時刻より前か」で判定する
        ...
        # GUIなどのコンソールで「3秒間に何回シミュレーションできたか」を確認したい場合は
        # 以下のコメントアウトを外すと便利です。
        # print(f"Time MCTS: {time_limit_sec}秒で {simulations_done} 回の探索を実行しました。")
